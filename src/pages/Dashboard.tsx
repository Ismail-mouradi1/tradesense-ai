...
export default function Dashboard() {
  ...
  const exportTradesCSV = () => {
    const headers = ['Symbol', 'Side', 'Quantity', 'EntryPrice', 'Status', 'CreatedAt'];
    const rows = trades.map((tr) => [
      tr.symbol,
      tr.side,
      String(tr.quantity),
      Number(tr.entry_price).toFixed(4),
      tr.status || '',
      new Date(tr.created_at).toISOString(),
    ]);
    const csv = [headers.join(','), ...rows.map((r) => r.join(','))].join('\n');
    const id = (activeChallenge || challengeOverride)?.id || 'challenge';
    downloadCSV(csv, `trades_${id}.csv`);
  };

  const exportHoldingsCSV = () => {
    const headers = ['Symbol', 'Quantity', 'AvgEntry', 'MarketPrice', 'Value', 'PL'];
    const rows = holdings.map((h) => [
      h.symbol,
      h.quantity.toFixed(4),
      h.avgEntry.toFixed(4),
      h.marketPrice.toFixed(4),
      h.value.toFixed(2),
      h.pl.toFixed(2),
    ]);
    const csv = [headers.join(','), ...rows.map((r) => r.join(','))].join('\n');
    const id = (activeChallenge || challengeOverride)?.id || 'challenge';
    downloadCSV(csv, `portfolio_${id}.csv`);
  };
  useEffect(() => {
    if (activeChallenge) {
      try {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      // Try to insert trade into database - if it fails due to RLS, continue with local state only
      let tradeInsertedToDb = false;
      try {
        const { error: tradeError } = await supabase.from('trades').insert(newTrade);
        if (!tradeError) {
          tradeInsertedToDb = true;
        } else {
          console.log('Trade insert to DB failed (continuing with local state):', tradeError.message);
        }
      } catch (dbError) {
        console.log('Trade insert exception (continuing with local state):', dbError);
      }
      ...
      // Try to update challenge balance in database (may fail due to RLS policy)
      // If it fails, we continue anyway - local state is already updated
      try {
        await supabase
        ...
        // Silently ignore update errors - balance tracked locally
        console.log('Challenge update skipped (RLS policy)');
      }

      // Only invalidate queries if trade was actually inserted to DB
      // This prevents refetching stale data that would override our local state
      if (tradeInsertedToDb) {
                ...
                <Card className="mt-6">
                  <CardHeader className="flex flex-row items-center justify-between">
                    ...
                    <Button variant="outline" size="sm" onClick={exportTradesCSV}>
                      {t.dashboard.labels.exportTrades}
                    </Button>
                  </CardHeader>
                  <CardContent>
                    {trades.length === 0 ? (
                ...
                <Card className="mt-6">
                  <CardHeader className="flex flex-row items-center justify-between">
                    ...
                    <Button variant="outline" size="sm" onClick={exportHoldingsCSV}>
                      {t.dashboard.labels.exportPortfolio}
                    </Button>
                  </CardHeader>
                  <CardContent>
                    {holdings.length === 0 ? (
